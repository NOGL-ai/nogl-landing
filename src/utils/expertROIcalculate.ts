...
export function calculateEarnings(data: EarningsData, isPlatform: boolean) {
	const {
		...
		administrativeTime,
		cancellationRate,
		platformFeeRate = 0.2, // Default to 20%
	} = data;
	...
	const totalRevenue =
		actualSessions * participantsPerSession * pricePerSession;
	...
	const totalTimeInvestment =
		actualSessions *
		(preparationTime +
			(isPlatform ? administrativeTime * 0.5 : administrativeTime));
	const effectiveHourlyRate = totalTimeInvestment
		? netIncome / totalTimeInvestment
		: 0;
