...
export function createOptimizedDataFetcher<TData = any>(
  ...
  const cache = new Map<string, { data: InfiniteScrollData<TData>; timestamp: number }>();

  return async (params: ServerSideParams): Promise<InfiniteScrollData<TData>> => {
...
export function createOptimizedPrismaFetcher<TData>(
  ...
  return async (params: ServerSideParams): Promise<InfiniteScrollData<TData>> => {
    ...
    try {
      ...
      const where = params.filters ? buildWhereClause(params.filters) : undefined;
...
export class PerformanceMonitor {
  ...
  static getStats(): Record<string, { average: number; count: number; min: number; max: number }> {
