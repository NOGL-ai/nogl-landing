...
export default async function ProductExplorerPage({ params, searchParams }: PageProps) {
  ...
  const { q, limit: limitStr } = await searchParams;
  ...
  try {
    products = await getScrapedProducts(limit, query || undefined);
  } catch (err) {
    ...
    <div className="flex flex-col gap-6 p-6 lg:p-8">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          Search and explore the scraped product catalog.
        </p>
      </div>

      {/* Search bar */}
      <form method="GET" action={`/${lang}/product-explorer`} className="flex w-full max-w-xl gap-2">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        {query ? (
          ...
          <span>Showing latest {products.length} products</span>
        )}
      </div>
      ...
      {products.length >= limit && (
        <div className="text-center">
          <Link
            href={`/${lang}/product-explorer?q=${encodeURIComponent(query)}&limit=${limit + 48}`}
