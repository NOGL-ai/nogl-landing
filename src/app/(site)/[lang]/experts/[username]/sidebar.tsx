...
const Sidebar: React.FC<SidebarProps> = ({ name, avatarUrl, rating, location, joinedDate }) => {
  return (
    <div className="flex w-full flex-col items-center space-y-6 border-neutral-200 px-0 text-center dark:border-neutral-700 sm:space-y-7 sm:rounded-2xl sm:border sm:p-6 xl:p-8">
      {/* Avatar */}
      <Avatar imgUrl={avatarUrl} hasChecked hasCheckedClass="w-6 h-6 -top-0.5 right-2" sizeClass="w-28 h-28" />
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-neutral-6000 dark:text-neutral-300">{location}</span>
        </div>
        <div className="flex items-center space-x-4">
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          <span className="text-neutral-6000 dark:text-neutral-300">Joined in {joinedDate}</span>
        </div>
      </div>
    </div>
  );
