			...
			const color =
				stock > 100
					...
					: stock > 20
						...
						: "text-red-600";
			return (
				<div className={`text-right font-mono text-sm ${color}`}>{stock}</div>
			);
		},
	},
	{
...
export function UltimateBigDataTable() {
	const [dataSource, setDataSource] = useState<"mock" | "api" | "prisma">(
		"mock"
	);
	...
	const baseDataFetcher = useMemo(() => {
		switch (dataSource) {
			...
			case "api":
				return createOptimizedDataFetcher<Product>("/api/products", {
					pageSize,
					maxCacheSize: 1000,
					staleTime: 5 * 60 * 1000,
				});
	...
	const dataFetcher = (pageParam: number) =>
		baseDataFetcher({ page: pageParam, pageSize });
	...
	const queryKey = useMemo(
		() => ["ultimate-big-data-products", dataSource, totalRecords, pageSize],
		[dataSource, totalRecords, pageSize]
	);
		...
		<div className='w-full space-y-6'>
			...
			<div className='rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800'>
				...
				<p className='text-gray-600 dark:text-gray-400'>
					High-performance table with virtual scrolling, infinite loading, and
			...
			<div className='rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20'>
				...
				<div className='grid grid-cols-1 gap-4 text-sm text-blue-800 md:grid-cols-2 lg:grid-cols-3 dark:text-blue-200'>
			...
			<div className='rounded-lg border border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800'>
				...
				<div className='space-y-3 text-sm text-gray-700 dark:text-gray-300'>
					<div>
						<strong>1. Data Source Selection:</strong> Choose between mock data,
						API endpoint, or Prisma database
					</div>
					<div>
						<strong>2. Record Count:</strong> Test with different dataset sizes
						to see performance scaling
					</div>
					<div>
						<strong>3. Page Size:</strong> Adjust how many records are fetched
						per request
					</div>
					<div>
						<strong>4. Sorting:</strong> Click column headers to sort (handled
						server-side)
					</div>
					<div>
						<strong>5. Scrolling:</strong> Scroll to automatically load more
						data
					</div>
					<div>
						<strong>6. Performance:</strong> Monitor real-time metrics in the
						header
					</div>
				</div>
			</div>
