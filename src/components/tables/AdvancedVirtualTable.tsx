...
interface AdvancedVirtualTableProps<TData> extends InfiniteScrollTableProps<TData> {
...
export function AdvancedVirtualTable<TData>({
  ...
  const table = useReactTable({
    ...
    onSortingChange,
    onRowSelectionChange: onRowSelect as unknown as OnChangeFn<RowSelectionState>,
  ...
  }, [fetchNextPage, isFetching, hasNextPage, threshold, enablePerformanceMonitoring, performanceMonitor]);
  ...
  const renderRow = useCallback(
    (virtualRow: any) => {
      ...
      const isSelected = selectedRows.has(rowKeyExtractor(row.original, virtualRow.index));
      ...
      return (
        <tr
          ...
          className={cn(
            'flex w-full cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors',
  ...
  useEffect(() => {
    if (enableMemoryOptimization && rowHeights.size > 1000) {
      ...
      for (let i = Math.max(0, currentIndex - 100); i < currentIndex + 200; i++) {
        if (rowHeights.has(i)) {
    ...
    <div className="w-full">
      ...
      <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
        <div className="flex items-center justify-between">
          <span>
            Showing {totalFetched.toLocaleString()} of {totalRowCount.toLocaleString()} rows
            {isFetchingNextPage && ' (Loading more...)'}
          </span>
          {enablePerformanceMonitoring && performanceMonitor && (
            <div className="text-xs text-gray-500">
              Avg render: {performanceMonitor?.getAverageTime('scroll-handler').toFixed(1)}ms
            </div>
          )}
        </div>
      </div>
          ...
          <div className="inline-flex items-center space-x-2 text-gray-600 dark:text-gray-400">
            ...
            {enablePerformanceMonitoring && performanceMonitor && (
              <span className="text-xs">
                ({performanceMonitor.getAverageTime('scroll-handler').toFixed(1)}ms avg)
              </span>
            )}
          </div>
        </div>
