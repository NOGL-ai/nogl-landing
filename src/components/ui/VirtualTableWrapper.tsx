...
export const VirtualTableWrapper: React.FC<VirtualTableWrapperProps> = ({
  ...
  const shouldVirtualize = enableVirtualization && data.length >= threshold && isClient;
  ...
  }, [data, columns, selectedRows, onRowClick, onRowDoubleClick, onRowSelect, renderRow, shouldVirtualize]);
  ...
  if (!shouldVirtualize) {
    return (
      <div className={cn('rounded-md border border-gray-200 dark:border-gray-700', className)}>
        <Table>
          <TableHeader className={cn(headerSticky && 'sticky top-0 z-10 bg-white dark:bg-gray-900')}>
            <TableRow>
              {columns.map((column, index) => (
                <TableHead key={index} style={{ minWidth: column.minWidth || '150px' }}>
                  {column.header}
                </TableHead>
              ))}
            </TableRow>
