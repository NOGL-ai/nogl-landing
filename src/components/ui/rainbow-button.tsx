...
export function RainbowButton({
  ...
  const useComplexEffects = hasGPU && (animationLevel === 'full' || animationLevel === 'reduced');
  ...
  if (animationLevel === 'none') {
    return (
      <button
        className={cn(
          "group relative inline-flex h-11 cursor-pointer items-center justify-center rounded-xl border-0 bg-[length:200%] px-8 py-2 font-medium text-primary-foreground transition-colors duration-200",
  ...
  return (
    <motion.button
      className={cn(
        ...
        useComplexEffects && [
          "before:absolute before:bottom-[-10%] before:left-1/2 before:z-0 before:h-1/6 before:w-2/5 before:-translate-x-1/2 before:bg-[linear-gradient(90deg,hsl(var(--color-1)),hsl(var(--color-5)),hsl(var(--color-3)),hsl(var(--color-4)),hsl(var(--color-2)))] before:bg-[length:200%] before:[filter:blur(calc(0.4*1rem))] before:transition-transform before:duration-300",
      ...
      style={{ 
        willChange: animationLevel === 'minimal' 
          ? 'opacity' 
          : 'transform, opacity' 
      }}
      {...props}
