  ...
  const columns: ColumnDef<MonitoredUrlRow>[] = useMemo(
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="space-y-1 max-w-[520px]">
              <div className="text-sm text-blue-600 truncate" title={r.competitorUrl}>{r.competitorUrl}</div>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right text-sm font-medium">{formatEuro(row.getValue("competitorPrice") as number | null)}</div>
        ),
        sortingFn: (a, b, id) => {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right text-sm">{formatEuro(row.getValue("myPrice") as number | null)}</div>
        ),
        sortingFn: (a, b, id) => {
          const av = (a.getValue(id) as number | null) ?? -Infinity;
          const bv = (b.getValue(id) as number | null) ?? -Infinity;
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const v = row.getValue("pricePosition") as MonitoredUrlRow["pricePosition"];
          const color = v === "Lower" ? "text-green-600" : v === "Higher" ? "text-red-600" : v === "Equal" ? "text-gray-600" : "text-blue-600";
          return <div className={`text-sm font-medium text-right ${color}`}>{v ?? "Analyzing..."}</div>;
        },
      },
      {
        accessorKey: "stock",
        ...
        cell: ({ row }) => {
          ...
          return <div className="text-sm text-right">{typeof v === "number" ? v : v ?? "Analyzing..."}</div>;
        },
      },
      {
        accessorKey: "status",
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex justify-end">
              <Badge variant={isActive ? "default" : isProgress ? "secondary" : "outline"}>
                {isActive ? (
            ...
            <div className="flex justify-end">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuCheckboxItem onClick={() => console.log("View", r.id)}>
                    <Eye className="mr-2 h-4 w-4" />
                    View
                  </DropdownMenuCheckboxItem>
                  <DropdownMenuCheckboxItem onClick={() => console.log("Edit", r.id)}>
                    <Edit className="mr-2 h-4 w-4" />
                    Edit
                  </DropdownMenuCheckboxItem>
                  <DropdownMenuCheckboxItem onClick={() => console.log("Delete", r.id)} className="text-red-600">
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between py-4">
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-6 lg:gap-8">
          <div className="flex items-center gap-2">
            <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Rows</p>
            <select
              className="h-8 w-[70px] bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md text-sm"
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              className="h-8 px-2 border-gray-300 dark:border-gray-600"
