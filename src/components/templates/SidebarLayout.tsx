...
const SidebarLayout: React.FC<SidebarLayoutProps> = ({
	...
	// Flatten all navigation items for the NavList
	const allNavItems = navItemsWithSectionsSubheadings.flatMap(section => section.items);

	return (
		<div className={`flex h-screen ${className}`}>
			{/* Desktop Sidebar */}
			<div className='fixed left-0 top-0 z-50 hidden lg:block'>
				<div className={`flex h-full bg-primary transition-all duration-300 ease-in-out ${
					isCollapsed && !isHovered ? "w-24" : "w-72"
				}`}>
					<div className="flex flex-col w-full">
						{/* Logo */}
						<div className="flex items-center justify-center p-4 border-b border-secondary">
							...
							{(!isCollapsed || isHovered) && (
								<span className='ml-2 text-lg font-semibold text-secondary'>NOGL</span>
							)}
						</div>

						{/* Navigation */}
						<div className="flex-1 overflow-y-auto">
							<NavList activeUrl={pathname} items={allNavItems} />
						</div>
						...
						{user && (
							<div className="p-4 border-t border-secondary">
								<NavAccountCard 
									selectedAccountId="current-user"
									items={[{
										id: "current-user",
										name: user.name,
										email: user.email,
			...
			{/* Mobile Sidebar - Keep existing for now */}
			<div className={`lg:hidden fixed inset-0 z-50 ${isMobileOpen ? 'block' : 'hidden'}`}>
				...
				<div className="fixed left-0 top-0 h-full w-72 bg-primary">
					<div className="flex flex-col h-full">
						...
						<div className="flex items-center justify-between p-4 border-b border-secondary">
							<div className="flex items-center gap-2">
								...
								<span className='text-lg font-semibold text-secondary'>NOGL</span>
							</div>
							<button
								type='button'
			...
			<div
				className={`flex flex-1 flex-col overflow-hidden transition-all duration-300 ease-in-out ${
					isCollapsed && !isHovered ? "lg:ml-24" : "lg:ml-72"
				}`}
			>
				{/* Mobile header */}
