...
const CommunitySection: React.FC<CommunitySectionProps> = ({
  ...
  stickyOffset = "80px",
  tabs = [
    { name: "News Feed", icon: <FiHome className="w-5 h-5" />, content: undefined },
    { name: "Members", icon: <FiUsers className="w-5 h-5" />, count: memberCount, content: undefined },
    { name: "Calendar", icon: <FiCalendar className="w-5 h-5" />, content: undefined }
  ]
}) => {
  return (
    <div className={`listingSection__wrap ${className}`}>
      <Tab.Group>
        <div className={`sticky top-[${stickyOffset}] z-10 bg-white dark:bg-neutral-900`}>
          <Tab.List className="flex space-x-1 rounded-xl bg-neutral-100/70 p-1 dark:bg-neutral-800/70 backdrop-blur-sm">
            {tabs.map((tab) => (
              <Tab
                key={tab.name}
                className={({ selected }) =>
                  `flex items-center justify-center space-x-2 w-full rounded-lg py-3 text-sm font-medium leading-5 transition-all duration-150 ease-in-out
                  ${
                    selected
