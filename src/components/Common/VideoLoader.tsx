...
const VideoLoader = ({ 
  ...
  const shuffle = async () => {
    while (true) {
      ...
      animate(`[data-block-id="${first.id}"]`, { y: -BLOCK_SIZE } as any, TRANSITION);
      await animate(`[data-block-id="${second.id}"]`, { y: BLOCK_SIZE } as any, TRANSITION);
      ...
      await animate(`[data-block-id="${second.id}"]`, { y: 0 } as any, TRANSITION);
      await delay(DURATION_IN_MS);
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-neutral-100 dark:bg-black dark:bg-opacity-20">
