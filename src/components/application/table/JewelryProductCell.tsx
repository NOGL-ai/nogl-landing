    ...
    <div className={`group relative flex items-start ${sizeClasses.container} rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${className}`}>
      ...
      <div className="relative flex-shrink-0">
        <div className={`relative ${sizeClasses.image} rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 p-1 shadow-sm`}>
          ...
          />
          <div 
            className={`${sizeClasses.imageInner} rounded-md bg-gray-200 dark:bg-gray-600 items-center justify-center font-semibold text-gray-600 dark:text-gray-300 hidden`}
          ...
          {/* Status indicator */}
          {showStatus && (
            <div className={`absolute ${sizeClasses.statusIndicator} rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 flex items-center justify-center`}>
              <span 
      ...
      <div className="min-w-0 flex-1 space-y-1">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          {showPrice && (
            <div className="text-right flex-shrink-0">
              <div className={`${sizeClasses.price} font-bold text-gray-900 dark:text-gray-100`}>
                {product.myPrice.toLocaleString('de-DE', { 
                  style: 'currency', 
                  currency: product.currency || 'EUR' 
                })}
              </div>
              <div className={`${sizeClasses.priceLabel} text-gray-500 dark:text-gray-400`}>Your price</div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className={`${sizeClasses.subtitle} text-gray-500 dark:text-gray-400 font-mono`}>#{product.sku}</span>
            {showCompetitorCount && (
              <>
                <div className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></div>
                <span className={`${sizeClasses.subtitle} text-gray-500 dark:text-gray-400`}>
      ...
      {showTooltip && (
        <div className={`absolute left-0 top-full mt-2 hidden group-hover:block z-50 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 rounded-lg shadow-lg whitespace-nowrap ${sizeClasses.tooltip}`}>
          <div className="space-y-1">
            <div className="font-semibold">{product.name}</div>
            <div className="text-gray-300 dark:text-gray-600">{product.brand.name}</div>
            <div className="text-gray-300 dark:text-gray-600">SKU: {product.sku}</div>
            {showPrice && (
            ...
            {showCompetitorCount && (
              <div className="text-gray-300 dark:text-gray-600">{product.competitorCount} competitors monitoring</div>
            )}
