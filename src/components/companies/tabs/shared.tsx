      ...
      <div className={`grid gap-4 ${grid}`}>
        {Array.from({ length: rows }).map((_, index) => (
          <Card key={index} className="animate-pulse p-5">
            ...
            <div className="mt-4 h-8 w-16 rounded bg-bg-tertiary" />
          </Card>
        ))}
      </div>
      <Card className="animate-pulse p-6">
        ...
        <div className="mt-6 space-y-3">
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="h-10 rounded bg-bg-tertiary" />
          ))}
        </div>
      </Card>
    </div>
  ...
  return (
    <Card className="border-l-4 border-l-zinc-400 p-5 animate-pulse">
      ...
      <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        {/* Date */}
        <div className="h-4 w-32 rounded bg-bg-tertiary flex-shrink-0" />
      </div>

      {/* Image carousel area */}
      <div className="mt-4 flex justify-center">
        <div className="h-48 w-full rounded-lg bg-bg-tertiary" />
      </div>
...
export function InlineError({ message }: { message: string }) {
  return (
    <div className="rounded-xl border border-border-error bg-bg-error p-4 text-sm text-text-error">
      {message}
    </div>
  );
}
