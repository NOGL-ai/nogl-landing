    ...
    <div className="overflow-hidden rounded-xl border border-border-primary bg-bg-primary p-6 shadow-xs">
      <div className="animate-pulse space-y-4">
        <div className="grid gap-3 md:grid-cols-4">
          {Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-10 rounded-md bg-bg-tertiary" />)}
        </div>
        <div className="space-y-2">
          {Array.from({ length: 8 }).map((_, row) => (
            <div key={row} className="grid grid-cols-6 gap-2">
              {Array.from({ length: 6 }).map((_, col) => <div key={col} className="h-10 rounded-md bg-bg-tertiary" />)}
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        <p className="mb-2 text-sm text-text-tertiary">
          <span className="font-medium text-text-secondary">Presets</span>{" "}
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-xs font-medium shadow-xs transition-colors ${
                  isActive
                    ...
                    : 'border-border-primary bg-bg-primary text-text-secondary hover:bg-bg-secondary hover:border-border-secondary hover:text-text-primary active:bg-bg-tertiary'
                }`}
              >
                {preset.label}
              </button>
              ...
              <tbody>
                {matrix.rows.map((row) => (
                  <tr key={row}>
                      ...
                      return (
                        <td
                          ...
                          className={`border-b border-border-primary px-4 py-3 text-right tabular-nums transition-colors ${
                            heat.bg ? `${heat.bg} font-semibold ${heat.text}` :
                            value === null ? 'text-text-disabled' : 'text-text-primary'
                          }`}
                        >
                    ...
                    <td className="border-b border-border-primary px-4 py-3 text-right font-semibold tabular-nums text-text-primary">{formatValue(metric, rowTotals.get(row) ?? null)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  ...
                  {matrix.cols.map((col) => (
                    <td key={`total-${col}`} className="border-t border-border-primary bg-bg-secondary px-4 py-3 text-right font-semibold tabular-nums text-text-primary">{formatValue(metric, colTotals.get(col) ?? null)}</td>
                  ))}
                  <td className="border-t border-border-primary bg-bg-secondary px-4 py-3 text-right font-semibold tabular-nums text-text-primary">{formatValue(metric, grandTotal)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
