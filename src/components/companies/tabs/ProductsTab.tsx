...
function ProductCardSkeleton() {
  return (
    <div className="animate-pulse overflow-hidden rounded-xl border border-border-primary bg-bg-primary">
      ...
      >
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-white border-b border-border-primary">
          {product.image_url ? (
            ...
            <div className="flex h-full items-center justify-center text-text-disabled/30">
              <svg
                className="h-12 w-12"
                fill="none"
                viewBox="0 0 24 24"
          ...
          {product.discount_pct != null && product.discount_pct > 0 && (
            <span className="absolute right-2 top-2 rounded-full bg-bg-error px-2 py-0.5 text-xs font-semibold text-text-error">
              -{Math.round(product.discount_pct)}%
            </span>
          )}

          {/* Hover overlay */}
          <div className="absolute inset-0 flex items-center justify-center bg-bg-primary/0 transition-all group-hover:bg-bg-primary/[0.08]">
            <span className="scale-90 rounded-full bg-bg-primary/90 px-3 py-1.5 text-xs font-medium text-text-primary opacity-0 shadow-sm transition-all group-hover:scale-100 group-hover:opacity-100">
              View details →
            </span>
          </div>
        </div>
        ...
        <div className="p-3 space-y-1">
          ...
          {product.category && (
            <span className="inline-block rounded-full bg-bg-tertiary px-2 py-0.5 text-xs text-text-tertiary">
              {product.category}
            </span>
          )}
  ...
  return (
    <Link href={href as `/${string}`}>
      ...
      >
        ...
        {/* Discount badge */}
        {hasDiscount && (
          <span className="shrink-0 rounded-full bg-bg-error px-2 py-0.5 text-xs font-semibold text-text-error">
            -{Math.round(product.discount_pct!)}%
          </span>
        )}
        ...
        <div className="shrink-0 text-right">
          ...
          <p className="text-xs text-text-tertiary">{timeAgo(product.first_seen)}</p>
        </div>
      </article>
    </Link>
  );
...
function ProductRowSkeleton() {
  return (
    <div className="animate-pulse flex items-center gap-4 rounded-xl border border-border-primary bg-bg-primary px-4 py-3">
      <div className="h-12 w-12 shrink-0 rounded-lg bg-bg-tertiary" />
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full rounded-lg border border-border-primary bg-bg-primary py-2 pl-10 pr-4 text-sm text-text-primary placeholder:text-text-quaternary focus:outline-none focus:ring-2 focus:ring-border-brand"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-24 rounded-lg border border-border-primary bg-bg-primary px-3 py-2 text-sm text-text-primary placeholder:text-text-quaternary focus:outline-none focus:ring-2 focus:ring-border-brand"
          />
        ...
        {hasFilters && (
          <button
            ...
            className="rounded-lg border border-border-primary px-3 py-2 text-sm text-text-tertiary transition-colors hover:bg-bg-secondary hover:text-text-primary"
          >
            Clear filters
          </button>
        )}
        ...
        {/* View mode toggle */}
        <div className="ml-auto flex items-center rounded-lg border border-border-primary shadow-xs p-1">
          <button
            onClick={() => setViewMode("grid")}
            title="Grid view"
            className={`rounded p-1.5 transition-colors ${viewMode === "grid" ? "bg-brand-50 text-text-brand" : "text-text-tertiary hover:text-text-primary"}`}
          >
          ...
          <button
            ...
            title="List view"
            className={`rounded p-1.5 transition-colors ${viewMode === "list" ? "bg-brand-50 text-text-brand" : "text-text-tertiary hover:text-text-primary"}`}
          >
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              !category
                ? "bg-brand-50 text-text-brand border border-border-brand"
      ...
      {state.loading ? (
        ...
        <div className="rounded-xl border border-border-primary bg-bg-secondary py-16 text-center">
          ...
          {hasFilters && (
            <button
              onClick={clearFilters}
              className="mt-3 text-xs text-text-tertiary underline hover:text-text-primary"
            >
              Clear filters
            </button>
          )}
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="flex items-center justify-between pt-2">
          <button
            ...
            className="rounded-lg border border-border-primary px-4 py-2 text-sm font-medium text-text-primary transition-colors hover:bg-bg-secondary disabled:cursor-not-allowed disabled:opacity-40"
          >
            Previous
          </button>
