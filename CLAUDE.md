...
## 🎯 Calumet Forecast Seed (Wave 2)

### Quick run

From CT 504 (preferred — server-side, fast):
```bash
ssh nogl-dev "cd /tmp && rm -rf calumet-seed && git clone --depth 50 http://10.10.10.171:3000/nogladmin/nogl-landing.git calumet-seed && cd calumet-seed && git checkout main && ln -sf /root/nogl-landing/node_modules ./node_modules && cp /root/nogl-landing/.env . && npx prisma generate && npx tsx scripts/seed-calumet-realistic.ts --wipe --horizon-days=180"
```

From a local machine (slower, ~35-45 min due to Postgres RTT):
```bash
npx tsx scripts/seed-calumet-realistic.ts --wipe --horizon-days=180
```

Smoke mode (50 SKUs, ~6 min):
```bash
npx tsx scripts/seed-calumet-realistic.ts --wipe --smoke --horizon-days=180
```

### File map

- `scripts/seed-calumet-realistic.ts` — orchestrator
- `scripts/seed-calumet/load-instax.ts` — CSV loader (Instax)
- `scripts/seed-calumet/load-amazon.ts` — CSV loader (Amazon Camera & Photo)
- `scripts/seed-calumet/hero-skus.ts` — 7 hero SKUs (X100VI, Canon R5 II, etc.)
- `scripts/seed-calumet/build-catalog.ts` — merges into ForecastProduct + ForecastVariant
- `scripts/seed-calumet/generate-history.ts` — Poisson + AR(1) + sinusoidal + German events
- `scripts/seed-calumet/inject-stockouts.ts` — bursty stockouts + restock spike
- `scripts/seed-calumet/generate-quantiles.ts` — forward forecast with rrp-conditional spreads
- `scripts/seed-calumet/seed-annotations.ts` — 6 ForecastAnnotation rows
- `scripts/seed-calumet/math-helpers.ts` — Poisson sampler, Gaussian noise, etc.
- `scripts/validate-seed.ts` — post-seed validation queries

### CSV inputs (gitignored, manually downloaded)

- `scripts/data/fujifilm-instax.csv` (1.3MB, 12,334 rows)
- `scripts/data/amazon-electronics-2025.csv` (35MB, 42,675 rows; 3,677 in Cameras subcategory)

### Key constants

In `src/config/forecast.ts`:
- `FORECAST_HISTORY_DAYS = 730` (visible chart history window)
- `FORECAST_HORIZON_DAYS = 180` (forecast quantile forward window)
- `QUANTILE_MULTIPLIERS = {3: 0.65, 4: 1.0, 5: 1.35}` (P30 / P50 / P70 spreads)
- `CALUMET_CHANNEL_WEIGHTS = { shopify: 0.30, offline: 0.22, b2b: 0.18, amazon: 0.15, web: 0.10, marketplace: 0.05 }`

### Hero SKUs

| SKU | Story | Date(s) |
|---|---|---|
| Fujifilm X100VI | OOS gap | 2025-08-12 to 2025-09-28 |
| Canon EOS R5 Mark II | Launch spike | 2025-07-17 (+14d window) |
| Sony Alpha 7 IV | Steady baseline | n/a |
| Sigma 35mm f/1.4 DG DN Art | Lens companion to R5 II | n/a |
| Godox V1 Pro (Canon) | High-volume low-AOV | n/a |
| Manfrotto 055XPRO3 Tripod | Christmas spike | Dec 1-24 each year |
| SanDisk CFexpress 256GB | Commodity volume | n/a |

### Annotations seeded

- Black Friday 2024 (2024-11-29)
- Black Friday 2025 (2025-11-28)
- X100VI Stock Gap (2025-08-12 to 2025-09-28)
- Canon EOS R5 II Announce (2025-07-17)
- Valentine's Portrait Promo (2025-02-10 to 2025-02-14)
- Photokina-adjacent Pro Pulse (2025-09-20)

### Live DB column added in this session

`forecast.ForecastHistoricalSale.isStockout BOOLEAN NOT NULL DEFAULT false` — applied via raw ALTER TABLE while table was empty (zero data loss). The column is in `prisma/schema.prisma` line 847.

### Known schema drift

`prisma migrate diff` against the live DB shows multi-table drift (enum changes, schema creates, etc.) beyond the single isStockout addition. NOT addressed in this session. Audit at `.claude/research/schema-drift-audit-2026-04-26.md` (if generated by the parallel audit agent tonight). Recommendation: dedicated migration session — don't run `prisma migrate dev` until drift is reconciled.
