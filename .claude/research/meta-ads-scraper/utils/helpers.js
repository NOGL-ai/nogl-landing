...
// Lazily resolve Apify's `log` so this module can be required in test
// environments that don't have the apify SDK installed. Falls back to
// console for warnings.
let log;
try {
    ({ log } = require('apify'));
} catch (_) {
    log = { warning: (m) => console.warn(m), info: (m) => console.log(m) };
}
...
class DataProcessor {
    ...
    /**
     * Normalize a row from the official Ad Library API into the shared
     * record shape used by both the browser path and the dataset
     * writer.
     */
    static fromOfficialApi(row) {
        if (!row || typeof row !== 'object') return null;
        const spendLower = row.spend?.lower_bound;
        const spendUpper = row.spend?.upper_bound;
        const imprLower = row.impressions?.lower_bound;
        const imprUpper = row.impressions?.upper_bound;
        return {
            adId: row.id || null,
            pageId: row.page_id || null,
            pageName: row.page_name || null,
            adContent: Array.isArray(row.ad_creative_bodies) ? row.ad_creative_bodies[0] || '' : '',
            startDate: row.ad_delivery_start_time || null,
            endDate: row.ad_delivery_stop_time || null,
            spend: {
                min: spendLower != null ? Number(spendLower) : null,
                max: spendUpper != null ? Number(spendUpper) : null,
                currency: row.currency || null,
            },
            impressions: {
                min: imprLower != null ? Number(imprLower) : null,
                max: imprUpper != null ? Number(imprUpper) : null,
            },
            targeting: {
                ageMin: Array.isArray(row.target_ages) ? row.target_ages[0] ?? null : null,
                ageMax: Array.isArray(row.target_ages) ? row.target_ages[1] ?? null : null,
                genders: row.target_gender || null,
                locations: row.target_locations || null,
            },
